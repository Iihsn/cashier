...
app.post("/setStatusTransaksi", async (req, res) =>{
  const id_transaksi = req.body.id_transaksi;
  const id_meja = req.body.id_meja;

  await meja.update({ status: "kosong" }, { where: { id_meja: id_meja } })
  await transaksi.update({status : "lunas"}, {where :{id_transaksi:id_transaksi}}).then((result) =>{
